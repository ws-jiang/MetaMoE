...
class RESISC45:
    def __init__(self,
        ...
        self.dataset = RESISC45Dataset(root=root, split=split, transforms=transform)
        self.classes = [' '.join(c.split('_')) for c in RESISC45Dataset.classes]

    def __len__(self) -> int:
        return len(self.dataset.samples)

    def __getitem__(self, idx):
        image_file, label = self.dataset.samples[idx]
